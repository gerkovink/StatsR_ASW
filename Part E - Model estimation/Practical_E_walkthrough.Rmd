...
7. **What is the conclusion when the outcome is classified according to numbers of patients who experienced a greater than 50% reduction in headaches over a four-week period, relative to a pre-randomization baseline?**
...
8. **Patients who received the acupuncture and sham acupuncture treatments were asked to guess their treatment at the end of their trial. What would you conclude from this data?**
...
9. **Fit and inspect a model where `brw` is modeled from `bw`**
```{r}
mammalsleep %$%
  lm(brw ~ bw) %>%
  anova()
...
10. **Now fit and inspect a model where `brw` is predicted from both `bw` and `species`**
```{r}
mammalsleep %$%
  lm(brw ~ bw + species) %>%
  anova()
...
11. **Can you find a model that improves the $R^2$ in modeling `brw`?**
...
12. **Inspect the diagnostic plots for the model obtained under ***11***. **
```{r}
fit %>%
  plot(which = 1:6)
```
...
The following two exercises are particularly useful if you run your analysis models in R. The exercises demonstrate that it is possible to call other software from within `R`: in this case the command-line interface for Mplus. This is very useful as your data collecting, manipulation and editing can be flexibly done in `R`, while still allowing for seamless integration with unique software applications, such as Mplus. 

---

13. **Load package `mice` and export data frame `boys` to Mplus using function `prepareMplusData()` from package `MplusAutomation`. Name the exported data set `boys.dat`. Export only the columns that hold information about age, bmi, height, weight and head circumference.**

```{r warning=FALSE, message=FALSE}
require(mice)
require(MplusAutomation)
prepareMplusData(boys, filename="boys.dat", 
                       keepCols = c("age", "bmi", "hgt", "wgt", "hc"))
```
We see that the function `prepareMplusData()` automatically prepares the body for an input file. Only the columns named `age`, `bmi`, `hgt`, `wgt` and `hc` are included in the exported file. The argument `keepCols` takes care of that. Because we do not specify a path (i.e. a location where the file will be stored), but only specify the filename, the data set `boys.dat` is automatically exported to the working directory. 

To find out the current working directory, type:
```{r eval=FALSE}
getwd()
```
...
In the dropbox folder are three Mplus input files: `model1.inp`, `model2.inp` and `model3.inp`. One of the nice things about MplusAutomation is that it simplifies running batches of models. 

---

14. **Place the three input files in the same directory as the `boys.dat` file and run function `runModels()` on this directory.** Tip: if you place it in the working directory (or, equivalently, in the project directory), the below code will always run.

```{r warning=FALSE}
runModels()
```

The `runModels()` function takes several arguments. However, if nothing is specified by the user, the function will by default run all models it finds in the working directory. This is what happened in our case. 

If you check your working directory, you will see that an output file has been generated by Mplus for each of your models. 

Using `MplusAutomation` can be very handy, especially when running and comparing multiple models is the goal. The package runs Mplus from the command line, and without the computational overhead from running the graphical user interface of Mplus, running large models and large quantities of models may be significantly speedier than running the models one by one by hand. 
